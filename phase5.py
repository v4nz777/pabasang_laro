...
class CVCandE(MDScreen):
    ...
    def add_consonants(self, consonants_list):
        ...
        for i in consonants_list:
            ...
            self.label=MDLabel()
            self.label.text=i
            self.label.font_name='NotoSerif-Regular.ttf'
...
class CBlank(Scatter, MagicBehavior):
    ...
    def create(self):
        ...
        with self.canvas:
            ...
            self.line = Line(width=3,rectangle=(self.x + 10, self.y + 10, self.width, self.height))
        with self.canvas:
            self.rect_color = Color(rgba=(0.5, 0, 0, 1))
            self.rect = Rectangle(size=(self.size[0]+10,self.size[1]+10), pos=self.pos)
        
        label = MDLabel()
        label.text = ""
        label.size_hint = (1,0.3)
        label.font_name = 'NotoSerif-Regular.ttf'
...
class VCe(Scatter, MagicBehavior):
    ...
    def create(self):
        ...
        with self.canvas:
            self.line_color = Color(rgba=(1, 1, 1, 1))
            self.line = Line(width=2,rectangle=(self.x + 10, self.y + 10, self.width, self.height))
...
class CVCCons(Scatter):
    ...
    def __init__(self, **kwargs):
        ...
        with self.canvas:
            Color(rgba=(1, 1, 1, 1))
            Line(width=2,rectangle=(self.x + 10, self.y + 10, self.width, self.height))
        with self.canvas.before:
            Color(rgba=(0, 0.3, 0.2, 1))
            Rectangle(size=(self.size[0]+10,self.size[1]+10), pos=self.pos)
