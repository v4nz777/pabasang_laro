...
class PhonogramAlpha(MDScreen):
    ...
    def __init__(self, **kwargs):
        ...
        for letter in PHONOGRAM_ALPHA:
            self.button = MDRectangleFlatButton(text=letter,font_size='30sp', size_hint_y=None, font_name='NotoSerif-Regular.ttf')
            self.button.bind(on_press=self.letter_sound)
            self.phonograms_alpha.add_widget(self.button)
        
        for letter in PHONOGRAM_MULTI_CT:
            self.button = MDRectangleFlatButton(text=letter,font_size='25sp', size_hint_y=None, font_name='NotoSerif-Regular.ttf')
            self.button.bind(on_press=self.letter_sound)
